...
class MountAzure(Mount):
    def __init__(self,
        ...
        Args:
            zone (str): Zone name. i.e. ''
            azure_bucket (str): Bucket name
            azure_path (str): Path underneath bucket. The full path will become
                https://{storage_name}.blob.core.windows.net/{azure_path}

        Note: Sync Azure (not working)
            output -> storage/doodad-data/azure_path
            logs   -> storage/doodad-logs/xxxx.log

        Note: Sync GCP (not working)
            output/* -> storage/exp_path/storage_path/logs/output/*
            .log   -> storage/exp_path/storage_path/xxxx.log
        ...
        if azure_path.startswith('/'):
            ...
            # We store everything into a fixed dir /doodad on the remote machine
            # so AZMode knows to simply sync /doodad-data
            # (this is b/c we no longer pass in mounts to the launch mode)
